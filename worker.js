    ...
    <style>
        ...
        * {
        ...
        #passwordForm {
            ...
            text-align: center;
            width: 20rem; /* Equivalent to 320px */
        }


        #passwordLabel {
            color: #ddd; /* Greyish text */
            display: block;
            margin-bottom: 0.5rem; /* Spacing below label - Equivalent to 8px */
            font-size: 1rem;
        }
    </style>
</head>
<body class="page-bg text-white">
    ...
    <div id="passwordOverlay">
        <div id="passwordForm" class="space-y-4">
            <label id="passwordLabel" for="passwordInput" class="block text-sm font-medium text-gray-400">请输入访问密码:</label>
            <input 
                type="password" 
                id="passwordInput" 
                class="w-full bg-[#222] border border-[#333] text-white px-3 py-2 rounded-lg focus:outline-none focus:border-white transition-colors"
            ...
            <button 
                id="passwordSubmit" 
                class="inline-block px-4 py-2 bg-white text-black font-medium rounded-lg hover:bg-gray-200 transition-colors text-sm"
            >
                确认
            </button>
        </div>
    ...
    <script>
        const PASSWORD = 'wbzdmm'; // 修改成你的密码, 如果为空则不校验
        ...
        window.onload = function() {
            if (PASSWORD && PASSWORD.trim() !== "") { // Only show password overlay if PASSWORD is set
                if (!isAuthenticated) {
                    document.getElementById('passwordOverlay').style.display = 'flex';
                    document.getElementById('passwordSubmit').addEventListener('click', authenticate);
